...
def planner_echo(cmd: ParsedCommand) -> ExecutionResult:
    ...
    return ExecutionResult(
        success=True, message=cmd.slots.get("text", "Task completed.")
    )
            ...
            return [
                {
                    "intent": "conversation",
                    "action": "chat",
                    "slots": {"text": "My LLM client is offline."},
                    "confidence": 1.0,
                }
            ]
...
"""
        ...
        while iteration < self.MAX_ITERATIONS:
            ...
            try:
                ...
                # Emit Thought Event
                bus.publish(
                    AgentEventTypes.THOUGHT,
                    AgentThoughtEvent(
                        agent_name="Planner", thought=thought, step=iteration
                    ).to_dict(),
                )
                ...
                if parsed.get("is_complete"):
                    final_message = (
                        parsed.get("action", {})
                        .get("slots", {})
                        .get("text", "Task completed.")
                    )
                    bus.publish(
                        AgentEventTypes.PLAN_COMPLETED, {"message": final_message}
                    )
                    break
                ...
                if action_data:
                    ...
                    # Emit Action Event
                    bus.publish(
                        AgentEventTypes.ACTION,
                        AgentActionEvent(
                            agent_name="Planner", tool=f"{intent}/{action}", args=slots
                        ).to_dict(),
                    )
                    ...
                    if (
                        ...
                        if (
                            result.data.get("exit_code") == 0
                            and result.data.get("tests_passed") is True
                        ):
                            logger.info(
                                "Deterministic verification passed. Terminating debugging loop."
                            )
                            final_message = "All tests passed successfully. The debugging loop is complete."
                            bus.publish(
                                AgentEventTypes.PLAN_COMPLETED,
