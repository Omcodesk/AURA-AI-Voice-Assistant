...
def handle_media(cmd: ParsedCommand) -> ExecutionResult:
    if not _HAS_PYAUTOGUI:
        return ExecutionResult(
            False, "Media control library is missing on this system."
        )

    action = cmd.action
    logger.info(
        "Media Service Attempt | Raw: '{}' | Action: '{}'", cmd.source_text, action
    )
    ...
    if key:
        try:
            pyautogui.press(key)
            logger.info(
                "Media Service Status | Success | Action: {} (key: {})", action, key
            )
            return ExecutionResult(True, msg)
        except Exception as exc:
            logger.error(
                "Media Service Status | Failed | Action: {} | Error: {}", action, exc
            )
