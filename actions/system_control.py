...
def handle_volume(cmd: ParsedCommand) -> ExecutionResult:
    if not _HAS_PYAUTOGUI:
        return ExecutionResult(
            False, "Volume control relies on pyautogui which is not installed."
        )
    ...
    logger.info(
        "Volume Service Attempt | Raw: '{}' | amount: {} | steps: {}",
        raw_intent,
        amount,
        steps,
    )
    ...
    try:
        if "up" in raw_intent or "increase" in raw_intent:
            ...
            return ExecutionResult(
                False, "I didn't catch whether to turn the volume up or down."
            )
...
def handle_brightness(cmd: ParsedCommand) -> ExecutionResult:
    if not _HAS_SBC:
        return ExecutionResult(
            False, "Brightness control is not available on this device."
        )
    ...
    try:
        ...
        if "up" in raw_intent or "increase" in raw_intent:
            ...
            return ExecutionResult(
                False,
                "I didn't catch if you wanted to increase or decrease the brightness.",
            )
    except Exception as exc:
