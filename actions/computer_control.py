...
def click_element(cmd: ParsedCommand) -> ExecutionResult:
    ...
    if not coords:
        return ExecutionResult(
            success=False, message=f"Could not find '{element_name}' on screen."
        )
...
def type_into_element(cmd: ParsedCommand) -> ExecutionResult:
    ...
    _verify_safety()
    bus.publish(
        "agent.action", {"tool": f"Typing '{text[:15]}...' into '{element_name}'"}
    )
    try:
...
def focus_window(cmd: ParsedCommand) -> ExecutionResult:
    ...
    try:
        ...
        if not windows:
            return ExecutionResult(
                success=False, message=f"Window '{window_name}' not found."
            )
