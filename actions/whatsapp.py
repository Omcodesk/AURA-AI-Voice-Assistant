...
def handle_send_whatsapp(cmd: ParsedCommand) -> ExecutionResult:
    ...
    if not number:
        return ExecutionResult(
            False, f"Contact {contact_name} is not in your configured contacts list."
        )
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                page.wait_for_selector(
                    '#pane-side, div[role="button"]:has-text("Use Here")', timeout=60000
                )
                ...
                return ExecutionResult(
                    False,
                    "WhatsApp Web needs you to login. Try saying 'Open WhatsApp' first to scan your QR code.",
                )
            ...
            try:
                ...
                browser.close()
                return ExecutionResult(
                    True, f"WhatsApp message sent to {contact_name}."
                )
...
def handle_open_whatsapp(cmd: ParsedCommand) -> ExecutionResult:
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info(
                "WhatsApp window opened. Browser will remain open for 5 minutes."
            )
            time.sleep(300)
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                ...
                browser.close()
                return ExecutionResult(
                    False, "Could not load your recent chats. You may need to log in."
                )
