...
def read_file(cmd: ParsedCommand) -> ExecutionResult:
    ...
    try:
        ...
        return ExecutionResult(
            success=True,
            message=f"Contents of {filepath} (Lines {start_line}-{end_idx}):\n```\n{content}\n```",
        )
...
def write_file(cmd: ParsedCommand) -> ExecutionResult:
    ...
    filepath = resolve_path(filepath)
    bus.publish(
        "agent.action", {"tool": f"Writing to file: {os.path.basename(filepath)}"}
    )
...
def patch_file(cmd: ParsedCommand) -> ExecutionResult:
    ...
    if not filepath or not search_block:
        return ExecutionResult(
            success=False, message="Missing filepath or search_block."
        )
    ...
    bus.publish(
        "agent.action", {"tool": f"Patching file: {os.path.basename(filepath)}"}
    )
    ...
    try:
        ...
        if search_block not in content:
            return ExecutionResult(
                success=False,
                message="The exact search block was not found in the file. Patch failed.",
            )
        ...
        bus.publish("system.file.modified", {"filepath": filepath})
        return ExecutionResult(
            success=True, message=f"Successfully patched {filepath}."
        )
    except Exception as e:
...
def list_directory(cmd: ParsedCommand) -> ExecutionResult:
    ...
    bus.publish(
        "agent.action", {"tool": f"Listing directory: {os.path.basename(dirpath)}"}
    )
    ...
    try:
        ...
        output = "\n".join(items)
        return ExecutionResult(
            success=True, message=f"Directory contents of {dirpath}:\n{output}"
        )
    except Exception as e:
