...
def handle_weather(cmd: ParsedCommand) -> ExecutionResult:
    ...
    logger.info(
        "Weather Service Attempt | Raw: '{}' | Extracted: '{}'",
        cmd.source_text,
        extracted_loc,
    )
    logger.info(
        "Weather Service Resolve | Target: '{}' | Source: '{}'",
        location,
        "Default Fallback" if is_fallback else "User Speech",
    )
    ...
    try:
        ...
        if not data.get("results"):
            logger.warning(
                "Weather Service Status | Failed | Geocoding failed for: '{}'", location
            )
            return ExecutionResult(
                False, f"I couldn't find the location {location} on the map."
            )
        ...
        spoken = (
            f"As of {spoken_time}, the weather in {city} is {temp} degrees Celsius, "
        ...
        return ExecutionResult(
            False, "I'm sorry, I couldn't reach the weather service right now."
        )
