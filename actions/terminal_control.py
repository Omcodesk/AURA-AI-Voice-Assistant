    ...
    def check_safety(self, command: str) -> tuple[bool, str]:
        ...
        for d in DANGEROUS_COMMANDS:
            ...
            if command.lower().startswith(d + " ") or command.lower() == d:
                return (
                    False,
                    f"Interceptor Blocked: '{d}' is flagged as dangerous. You must request user approval first.",
                )
        return True, ""
...
def approve_command(cmd: ParsedCommand) -> ExecutionResult:
    ...
    return ExecutionResult(
        success=True, message=f"Command '{command}' approved for execution."
    )
...
def run_command(cmd: ParsedCommand) -> ExecutionResult:
    ...
    try:
        ...
        if result.returncode == 0 and (
            "test" in command.lower() or "pytest" in command.lower()
        ):
            tests_passed = True
        ...
        return ExecutionResult(
            success=result.returncode == 0, message=structured_msg, data=data_payload
        )

    except subprocess.TimeoutExpired:
        return ExecutionResult(
            success=False, message=f"Command timed out after {timeout} seconds."
        )
