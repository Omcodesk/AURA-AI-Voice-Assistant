...
def handle_open_app(cmd: ParsedCommand) -> ExecutionResult:
    ...
    logger.info(
        "App Control Resolve | Canonical: '{}' | Command: '{}' | Method: 'start'",
        target,
        exe or "NOT_FOUND",
    )
    ...
    if not exe:
        ...
        from actions.browser_control import (
            _MAPPINGS as SITE_MAPPINGS,
            handle_open_website,
        )

        if target in SITE_MAPPINGS or ("." in target and " " not in target):
            logger.info(
                "App Control Fallback | Target: '{}' | Rerouting to Browser", target
            )
    ...
    try:
        ...
        logger.error(
            "App Control Status | Failed | Target: {} | Error: {}", target, exc
        )
        ...
        if target == "chrome":
            ...
            for path in common_paths:
                if os.path.exists(path):
                    logger.info(
                        "Chrome Fallback Attempt | Method: 'Direct Path' | Path: '{}'",
                        path,
                    )
        ...
        return ExecutionResult(
            False, f"I couldn't open {target}. The system reported: {str(exc)}"
        )
...
def handle_close_app(cmd: ParsedCommand) -> ExecutionResult:
    ...
    if not exe:
        return ExecutionResult(
            False, f"I don't have a configured process for {target}."
        )
