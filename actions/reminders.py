...
def handle_set_reminder(cmd: ParsedCommand) -> ExecutionResult:
    ...
    if not time_str:
        return ExecutionResult(
            False,
            "I didn't hear a specific time for the reminder. When should I remind you?",
        )
    ...
    if not target_dt:
        return ExecutionResult(
            False,
            f"I couldn't understand the time '{time_str}'. Please try saying it differently.",
        )
...
def handle_cancel_reminder(cmd: ParsedCommand) -> ExecutionResult:
    ...
    return ExecutionResult(
        False, "You don't have any active alarms or reminders to cancel."
    )
...
def handle_snooze(cmd: ParsedCommand) -> ExecutionResult:
    ...
    with Session(memory._engine) as s:
        ...
        logger.info(
            "Snoozed reminder [Prev ID:{}] for {} minutes", last_rem.id, SNOOZE_MINUTES
        )
